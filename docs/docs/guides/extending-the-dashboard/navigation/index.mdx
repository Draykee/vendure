...
The ids used throughout this guide (`catalog`, `sales`, `products`, `administrators` and the rest) are exported as `BUILT_IN_NAV_SECTION_IDS` and `BUILT_IN_NAV_ITEM_IDS`. Referring to them through those constants means a changed id keeps working, and a removed one is a compile error. A hard-coded string gives you neither: it just stops matching.
