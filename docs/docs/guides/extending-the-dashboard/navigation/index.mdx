...
When you only need to condition an entry you own, `isVisible` is the shorthand. It is available on nav sections and on route `navMenuItem` entries, and is ANDed with `requiresPermission`: both must pass for the entry to appear.
...
On a section, returning `false` hides the section and all of its items. A section is also dropped once every item inside it is hidden, since it would open onto nothing.
...
One consequence is worth stating plainly. `keepOnlyNavEntries` is a whitelist, so it hides entries added by other plugins just as readily as built-in ones:
