...
These ids, and the ids of the items inside each section, are exported as `BUILT_IN_NAV_SECTION_IDS` and `BUILT_IN_NAV_ITEM_IDS`. Prefer them over string literals wherever you target a built-in entry, so that a rename turns into a compile error in your plugin:
...
Where you declare the predicate depends on whose entry it is. Use `isVisible` directly on the entries your own plugin declares. To reach an entry your plugin did not declare, a built-in section or one belonging to another plugin, attach the predicate from the function form of `navSections`.

### Reaching another plugin's entries with navSections

The function form of `navSections` runs once, at registration, and sees the fully assembled config. It has no access to the logged-in administrator and does not need one. The `isVisible` predicate it attaches is evaluated later, on every nav render, with the user context.
...
defineDashboardExtension({
    navSections: config => keepOnlyNavItems(config, ['pos'], isFloorStaff),
});
```

[keepOnlyNavItems](/reference/dashboard/extensions-api/navigation#keeponlynavitems) hides every entry not named in the list, and naming a section keeps all of its items. The third argument applies that hiding only to the administrators the predicate matches. Without it, every entry not named is hidden from everybody.

The other helper, [setNavVisibility](/reference/dashboard/extensions-api/navigation#setnavvisibility), names the entries to condition instead of the entries to keep:
...
defineDashboardExtension({
    navSections: config =>
        setNavVisibility(
            config,
            [BUILT_IN_NAV_SECTION_IDS.Catalog, BUILT_IN_NAV_SECTION_IDS.Sales],
            ctx => !isFloorStaff(ctx),
        ),
});
```

Prefer both over spreading `isVisible` yourself, because a plain spread silently discards a predicate that another plugin already set on the same entry.
...
An id which matches no entry has no effect, and with `keepOnlyNavItems` one typo hides every other entry. The dashboard warns about unmatched ids in development. For built-in entries, `BUILT_IN_NAV_SECTION_IDS` and `BUILT_IN_NAV_ITEM_IDS` remove the possibility.
...
A `navSections` function runs only once, but it is handed the framework's own section objects. Mutating a section or its `items` array in place corrupts the menu for the rest of the session. Always return new objects, as `setNavVisibility` and `keepOnlyNavItems` do.
...
The ids used throughout this guide (`catalog`, `sales`, `products`, `administrators` and the rest) are exported as `BUILT_IN_NAV_SECTION_IDS` and `BUILT_IN_NAV_ITEM_IDS`. Referring to them through those constants turns a rename into a compile error in your plugin, instead of a predicate which quietly stops matching.

Restructuring is a different matter. A future release could move an item into another section or drop it, and no constant catches that. When you target a built-in entry, re-check the sidebar after upgrading.
